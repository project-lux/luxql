...
- Relationships using -> operator
- Numeric comparitors for field values: > >= < <= and =
...
class BinaryOp(ASTNode):
    ...
    def to_luxql(self, parent):
        bl = LuxBoolean(self.op, parent)
        self.left.to_luxql(bl)
        self.right.to_luxql(bl)
        return bl
...
class UnaryOp(ASTNode):
    ...
    def to_luxql(self, parent):
        bl = LuxBoolean(self.op, parent)
        self.operand.to_luxql(bl)
        return bl


class Term(ASTNode):
    def __init__(self, value, fields=[], comparitor=None):
        self.value = value
        self.fields = fields
    ...
    def __repr__(self):
        if self.fields:
            f = "->".join(self.fields)
            if self.comparitor:
                return f"Term({f}{self.comparitor}{self.value})"
    ...
    def to_json(self):
        if self.fields:
            ...
            if self.comparitor:
                result["_comp"] = self.comparitor
            return top
        else:
            fld = {"text": str(self.value)}
            if self.comparitor:
                fld["_comp"] = self.comparitor
            return fld

    def to_luxql(self, parent):
        if self.fields:
            # Make a chain of LuxRelationships
            top = None
            current_parent = parent
            for f in self.fields[:-1]:
                rel = LuxRelationship(f, current_parent)
                if top is None:
                    top = rel
                current_parent = rel
            ...
            # single LuxLeaf
            leaf = LuxLeaf("text", parent, str(self.value), comparitor=self.comparitor)
            return leaf
...
class TermList(ASTNode):
    ...
    def to_luxql(self, parent):
        if isinstance(parent, LuxBoolean):
            bl = parent
        else:
            bl = LuxBoolean("AND", parent=parent)
...
def p_term_field_comp_word(p):
    """term : field_chain COMPARATOR WORD"""
    p[0] = Term(p[3], fields=p[1], comparitor=p[2])


def p_term_field_comp_quoted(p):
    """term : field_chain COMPARATOR QUOTED_STRING"""
    p[0] = Term(p[3], fields=p[1], comparitor=p[2])
...
class BooleanQueryParser:
    ...
    def make_query(self, query_string, scope=None):
        api = LuxAPI(scope)
        result = self.parser.parse(query_string, lexer=self.lexer)
        return result.to_luxql(api)
...
if __name__ == "__main__":
    ...
    test_queries = [
        ...
        'a->b:simple AND complex->chain->with->many->levels:"complex value"date > 1200',
    ]
